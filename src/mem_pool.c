...
static bool pool_malloc(State *S, void **ptr, size_t nbytes) {
	uint size;
	poolp pool;
	block *bp;
	poolp next;
...
static bool pool_free(State *S, void *p) {
	poolp pool;
	pool = POOL_ADDR(p);
	block *lastfree;
	poolp next, prev;
...
bool pool_realloc(State *S, void **ptr, size_t nbytes) {
	void *bp;
	poolp pool;
	size_t size;
	sky_assert(*ptr);
...
//		size = nbytes;
    ...
    pool_malloc(S, &bp, nbytes);
	if (bp != NULL) {
		memcpy(bp, *ptr, nbytes);
		sky_assert(pool_free(S, *ptr));
	}
	*ptr = bp;
	return 1;
}
