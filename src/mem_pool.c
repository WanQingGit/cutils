...
static u_int64_t size_allocated = 0;
...
static arena_obj *sy_new_arena() {
    ...
    if (unused_arena_objects == NULL) {
        ...
        size_allocated += nbytes - narenas * sizeof(*arenas);
        assert(arenaobj);
        arenas = arenaobj;
    ...
    size_allocated+= ARENA_SIZE;
//	address = mem_alloc(S, NULL, 0, ARENA_SIZE);
    if (address == NULL) {
    ...
    qassert(POOL_SIZE * arenaobj->nfreepools == ARENA_SIZE);
...
static bool pool_malloc(void **ptr, size_t nbytes) {
    uint size;
    poolp pool;
    block *bp;
    poolp next;
    ...
    if (usable_arenas == NULL) {
        ...
        usable_arenas = sy_new_arena();//freepools为NULL，需初始化
        if (usable_arenas == NULL) {
            goto failed;
        }
    ...
    if (pool != NULL) {
        ...
        if (usable_arenas->nfreepools == 0) {
            ...
            assert(
                    ...
                    usable_arenas->pool_address <= (block *) usable_arenas->address + ARENA_SIZE - POOL_SIZE);
        }
...
bool pool_realloc(void **ptr, size_t nbytes) {
    void *bp;
    poolp pool;
    size_t size;
    qassert(*ptr);
...
//		size = nbytes;
    ...
    pool_malloc( &bp, nbytes);
    if (bp != NULL) {
        memcpy(bp, *ptr, nbytes);
        qassert(pool_free(*ptr));
    }
    *ptr = bp;
    return 1;
}
