...
#include "strutils.h"
#include "mem.h"
static qstr *skys_createlstr(State *L, size_t l, const char* str);
static qstr *skys_get(const char *str);
...
static void qbufAdd(qstrbuf *buffer, const char *s, int n) {
	if (n > 0) {
		...
		sky_error("cannot add a string of negative length", ERR_RUN);
	}
}
...
static void qsplit(qvec l, const char *str, const char *s) {
	...
	while ((ptr = strstr(str, s)) != NULL) {
		qstr *objs = STR.create(str, ptr - str);
		Arr.append(l, objs);
		str = ptr + len;
	}
	Arr.append(l, (arr_type) STR.create(str, strlen(str)));
}
void skytstr_resize(State *S, int newsize) {
	...
	if (newsize != tb->size) { /* grow table if needed */
		skym_reallocvector(S, tb->ht, tb->size, newsize, qstr*);
	}
	int mod = newsize - 1;
...
static qstr *gshrstr(State *S, const char *str, size_t l) {
	...
	if (g->strt.nuse >= g->strt.size && g->strt.size <= UINT_MAX / 2) {
		skytstr_resize(S, g->strt.size * 2);
...
static qstr *skys_get(const char *str) {
	uint i = cast(size_t,str) & (STRCACHE_N - 1); /* hash */
	int j;
...
static qstr *string_new(const char *str, size_t l) {
	if (l <= MAXSHORTLEN)
		...
		qstr *ts = skys_createlstr(_S, l, str);
		return ts;
	}
}
static qstr *skys_createlstr(State *S, size_t l, const char* str) {
...
void strt_destroy() {
	...
	for (int i = 0; i < size; i++) {
		...
		while (str) {
			qstr *s = str;
			str = str->hnext;
			skym_alloc_pool(_S, o2gc(s), sizeof(qstr) + (s->len + 1) + sizeof(GCObj),
					0);
		}
		strt->ht[i] = NULL;
	}
...
struct strApi STR = { string_new, skys_get, skytstr_resize, qbufAdd, qsub,
		qsplit, qindex };
