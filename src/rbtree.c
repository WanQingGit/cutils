...
#define freeNode(node) do{\
		...
		skym_alloc_pool(_S,node,sizeof(RBNode),0);\
	}}while(0)
...
// 创建红黑树，返回"红黑树的根"！
static RBTree* rb_create(Type typeKey, Type typeVal);

// 销毁红黑树
static void rb_destroy(RBTree **tree, void (*destructor)(rbtype, rbtype));
static void rb_clear(RBTree *t, void (*destructor)(rbtype, rbtype));
...
static RBTree* rb_create(Type typeKey, Type typeVal) {
	...
	if (nfreetree > 0) {
		...
		tree = skym_alloc_pool(_S, NULL, NULL, sizeof(RBTree));
	if (typeKey)
		tree->typeKey = typeKey;
	else
		tree->typeKey = typeInt;
	tree->typeVal = typeVal;
	tree->root = NULL;
	tree->length = 0;
	tree->multi = false;
	return tree;
...
static void rb_preorder(RBTree *tree) {
	...
	int top = 0;
	RBNode **stack = qmalloc2(RBNode*, size), *node;
	stack[top++] = tree->root;
	while (top > 0) {
		node = stack[--top];
		printf("%d ", node->key);
...
static RBNode* rb_nearest(RBTree *tree, rbtype key, rb_flag flag) {
	...
	case RB_NEAR:
		if (below && above) {
			if (abs(tree->typeKey->compare(below->key, key))
					> abs(tree->typeKey->compare(above->key, key))) //abs(below->key - key) > abs(above->key - key)
				return above;
			return below;
		}
...
static RBTree *rb_clone(RBTree *tree) {
	RBTree *newTree = rb_create(tree->typeKey, tree->typeVal);
	newTree->multi = tree->multi;
...
static void rbtree_insert(RBTree *tree, RBNode *node) {
	...
	while (x != NULL) {
		y = x;
		res = tree->typeKey->compare(x->key, node->key);
	...
	if (y != NULL) {
		res = tree->typeKey->compare(y->key, node->key);
...
static bool rb_insert(RBTree *tree, rbtype key, rbtype val, RBNode** ptr) {
	...
	if (tree->multi == false && (node = rb_search(tree, key)) != NULL) {
		if (ptr)
			*ptr = node;
		return false;
	}
...
//	if ((node = create_rbtree_node(key, NULL, NULL, NULL)) == NULL)
	if (nfreenode > 0) {
		...
		*node = nilNode;
		...
		node = skym_alloc_pool(_S, NULL, NULL, sizeof(RBNode));
	}
...
//static void rbtree_destroy(rbNode *node) {
//	if (node == NULL)
//		return;
//	if (node->left != NULL)
			...
			if (destructor) {
				if (iptr <= RB_FREE_MAX) {
					if (iptr & RB_FREE_FORCE) {
						...
						if (iptr & RB_FREE_VAL) {
							qfree(node->val);/*skym_alloc_pool(_S,node->val,1,NULL);*/
						}
					} else {
						if (iptr & RB_FREE_KEY) {
							t->typeKey->free(node->key);
						}
						if (iptr & RB_FREE_VAL) {
							t->typeVal->free(node->val);
						}
					}
				} else
 ...
 * 销毁红黑树
 */
static void rb_destroy(RBTree **tree_ptr, void (*destructor)(rbtype, rbtype)) {
	RBTree *t = *tree_ptr;
	if (t == NULL)
		return;
	rb_clear(t, destructor);
	if (nfreetree < MAXFREETREE) {
		free_trees[nfreetree++] = t;
	} else
		skym_alloc_pool(_S, t, sizeof(RBTree), NULL);
	*tree_ptr = NULL;
}
void rb_cache_clear() {
	for (int i = 0; i < nfreetree; i++) {
		skym_alloc_pool(_S, free_trees[i], sizeof(RBTree), NULL);
	}
	nfreetree = 0;
	for (int i = 0; i < nfreenode; i++) {
		skym_alloc_pool(_S, free_nodes[i], sizeof(RBNode), NULL);
	}
	nfreenode = 0;
}
